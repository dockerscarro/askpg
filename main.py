...
def insert_logs_pg(rows):
    ...
    with conn.cursor() as cur:
        ...
        insert_query = f"""
            ...
            SELECT * FROM (VALUES %s) AS v(message, pid, level, timestamp)
            ON CONFLICT (message, pid, level, timestamp) DO NOTHING
        """
        psycopg2.extras.execute_values(cur, insert_query, unique_rows)
...
uploaded_files = st.sidebar.file_uploader(
    ...
    help="Select or drag one or more log files to upload. Supported types: .txt, .pdf, .docx"
)

if uploaded_files:
    for uploaded_file in uploaded_files:
        if "uploaded_file_name" not in st.session_state or st.session_state.uploaded_file_name != uploaded_file.name:
            with st.spinner(f"📥 Parsing and inserting {uploaded_file.name} into database..."):
                parsed_entries = load_and_parse_file(uploaded_file)
                ensure_table_exists()
                st.info(f"Table is ready for {uploaded_file.name}")

                inserted_count = insert_logs_pg(parsed_entries)
                if inserted_count > 0:
                    st.success(f"✅ Inserted {inserted_count} new log entries from {uploaded_file.name}.")
                    st.session_state.indexed = False
                else:
                    st.info(f"ℹ️ No new entries inserted from {uploaded_file.name}. Skipping indexing.")
